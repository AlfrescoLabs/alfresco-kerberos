...
[dbmodules]
  openldap_ldapconf = {
          db_library = kldap
          ldap_kdc_dn = "$LDAP_USER"

          # this object needs to have read rights on
          # the realm container, principal container and realm sub-trees
          ldap_kadmind_dn = "$LDAP_USER"
...
EOF
  ...
  kdb5_ldap_util -D $LDAP_USER stashsrvpw -f /etc/krb5kdc/service.keyfile $LDAP_USER <<EOF
$LDAP_PASS
$LDAP_PASS
$LDAP_PASS
EOF
...
create_admin_user() {
  kadmin.local -q "addprinc -x $KERB_ADMIN_USER admin" <<EOF
$LDAP_PASS
$LDAP_PASS
EOF
